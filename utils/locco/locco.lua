...
local function generate_html(source, filename, sections, jump_to)
  local out = filename:gsub('%..*$', '.html')
...
local function generate_documentation(source, filename, jump_to)
  ...
  generate_html(source, filename, sections, jump_to)
end
...
for i=1, #arg do
  ...
  generate_documentation(arg[i], filename, jump_to)
end
local f, err = io.open('locco.css', 'wb')
if err then print(err) end
